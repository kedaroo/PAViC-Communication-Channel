...
const client = new Client({
  connectionString:
    "postgres://ornadhmtjwkgpc:1710749e9a24e2a97f3c23dd7ee4244b57216c585f409992e8c65d6bae181dcc@ec2-35-153-91-18.compute-1.amazonaws.com:5432/d3t6h6goqj9e1g",
  ssl: {
...
io.on("connection", (socket) => {
  ...
  socket.on("fetch users", (count) => {
    ...
    if (count >= 0) {
      ...
      client.query(
        `SELECT * FROM dummy_users WHERE id > ${count};`,
        (err, res) => {
          if (err) throw err;
  ...
  socket.on("fetch transactions", (count) => {
    ...
    if (count >= 0) {
      ...
      // client.query(`SELECT b_id, from_add, to_add, amount FROM transactions WHERE b_id > ${count};`, (err, res) => {
      client.query(
        `SELECT b_id, from_add, to_add, amount FROM transactions2 WHERE id > ${count};`,
        (err, res) => {
          if (err) throw err;
          io.to(socket.id).emit("update transactions", { data: res.rows });
          console.log("TRANSACTIONS SENT:");
  ...
  socket.on("fetch pending transactions", (callback) => {
    ...
    if (blockSize == 0) {
      ...
      client.query(
        "SELECT from_add, to_add, amount FROM pending_transactions2;",
        (err, res) => {
          if (err) throw err;
      ...
      client.query(
        "SELECT from_add, to_add, amount FROM pending_transactions2 ORDER BY id LIMIT $1;",
        [blockSize],
        (err, res) => {
          // removed desc limit
          if (err) throw err;
    ...
    client.query(
      ...
      (err, res) => {
        if (res.rowCount == 1) {
          ...
          console.log(res);
          client.query(
            "INSERT INTO pending_transactions2 (from_add, to_add, amount) VALUES ($1, $2, $3);",
            [from, to, amount],
            (err, res) => {
              if (err) throw err;
