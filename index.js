...
const client = new Client({
  connectionString: 'postgres://bcgndofopgnhue:55fd71e4db871a3158b1699f6768e439f64ca74c673cc73f31f3a2ffb36ac28a@ec2-52-86-2-228.compute-1.amazonaws.com:5432/d2koer7v92n21g',
  ssl: {
...
io.on('connection', socket => {
      ...
      client.query(
        ...
        (err, res) => {
          ...
          if (res.rowCount == 0) {
            ...
            client.query(
              `insert into users3 (user_id, picture) values ($1, $2);`,     //REQUIRES MODIFICATION
              ...
              (err, res) => {
                ...
                if (err) {
                  ...
                  console.log('New User SUCESSFULLY registered: ', userToken.name)
                  // io.emit("add new user", [userToken.sub, userToken.picture]);    //MODIFY THIS 
    ...
    socket.on("fetch users", count => {
      ...
      if (count >= 0) {
        ...
        client.query(`SELECT * FROM users3 WHERE id > $1;`,
        [count],
        (err, res) => {
          if (err) throw err;
    ...
    socket.on("fetch blocks", count => {
      ...
      if (count > 0) {
        ...
        client.query(`SELECT * FROM blocks3 WHERE id > $1;`,
        [count],
        (err, res) => {
          if (err) throw err;
    ...
    socket.on("fetch transactions", count => {
      ...
      if (count >= 0) {
        ...
        console.log('FETCHING TRANSACTIONS...')
        // client.query(`SELECT b_id, from_add, to_add, amount FROM transactions WHERE b_id > ${count};`, (err, res) => {
          client.query(`SELECT b_id, from_add, to_add, amount FROM transactions3 WHERE id > ${count};`, (err, res) => {
          if (err) throw err;
          io.to(socket.id).emit("update transactions", {data: res.rows});
          console.log('TRANSACTIONS SENT:')
          console.log({data: res.rows})
    ...
    socket.on("fetch pending transactions", (callback) => {
      ...
      if (blockSize == 0) {
        ...
        client.query('SELECT from_add, to_add, amount FROM pending_transactions3;', 
        (err, res) => {
            if (err) throw err;
          ...
          client.query('SELECT from_add, to_add, amount FROM pending_transactions3 ORDER BY id LIMIT $1;',
          [blockSize],
          (err, res) => { // removed desc limit
            if (err) throw err;
            console.log('Number of transactions sent: ', res.rowCount)
            io.to(socket.id).emit("pending transactions", {data: res.rows});
            console.log('PENDING TRANSACTIONS SENT:')
      ...
      client.query(
        ...
        (err, res) => {
          if (res.rowCount == 1) {
            ...
            console.log(res.rowCount)
            console.log(res)
            client.query('INSERT INTO pending_transactions3 (from_add, to_add, amount) VALUES ($1, $2, $3);', 
